...
export function registerPublicRoutes(app: Express) {
  ...
  //
  // `seed` reports whether seedDatabase() completed on this process's boot. An
  // aborted seed leaves the server serving traffic with content missing — the
  ...
  app.get("/health", async (req, res) => {
    ...
    try {
      ...
      res.json({ status: seed === "failed" ? "degraded" : "ok", seed });
    } catch (error) {
