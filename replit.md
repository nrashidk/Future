...
- **Arabic / i18n Infrastructure (Task #11 + #12)**: i18next + react-i18next + i18next-http-backend installed; `client/src/i18n/config.ts` sets up lazy-loading locale JSON from `/locales/{lng}/common.json`; `LanguageContext` manages language state (localStorage for guests, DB `preferred_language` column for logged-in users), sets `html[lang]` and `html[dir]` for RTL; EN/AR toggle button in Header (desktop + mobile); Cairo font added to Google Fonts load; `[lang="ar"]` CSS rule switches `--font-sans/serif/body` to Cairo; `users.preferred_language` varchar column + `system_announcements.title_ar/content_ar` columns added to DB; `PATCH /api/users/me/language` backend endpoint; AnnouncementBanner serves AR text when language=ar; LLM service supports `{{language}}` template variable. Task #12 (assessment flow + results AR): RiasecStep 30 items + THEME_INFO + LIKERT fully translated inline; CVQStep DOMAIN_INFO + LIKERT use t() keys; QuizStep scoring bug fixed (always submits canonical English option text regardless of display language); Results.tsx + ResultsPrint.tsx render `titleAr`/`descriptionAr` for career cards; `mapSubjectsToVisionSectors` uses i18n `visionLinkageText` key; 39 careers seeded with `title_ar` + `description_ar` in DB.
