...
- **Arabic / i18n Infrastructure (Tasks #11–#14)**: i18next + react-i18next + i18next-http-backend with lazy-loading locale JSON; `LanguageContext` manages EN/AR state, sets `html[lang]` and `html[dir]` for RTL; Cairo font for Arabic; EN/AR toggle in Header. Assessment flow (Task #12): RiasecStep, CVQStep, QuizStep fully translated; Results/ResultsPrint render `titleAr`/`descriptionAr`. Admin i18n (Task #14): `admin` namespace (`client/public/locales/{en,ar}/admin.json`, ~760 lines each); all admin-facing pages use `useTranslation('admin')` with t() — AdminOrganizations, Admin (quiz/analytics tabs), SuperadminDashboard, and sub-components (ScoringConfigEditor, ContributeQuestions, ContributionReviewQueue, CountryManagement, SubjectManagement); RTL-safe `me-`/`ms-`/`pe-`/`ps-` spacing throughout; Translation Manager UI in SuperadminDashboard (GET/PATCH `/api/superadmin/translations/:namespace`, GET `.../coverage`).
